...
pub fn generate_with(seed: u64, opts: &GenOptions) -> CaveMap {
    ...
    // Fusion connectors are derived HERE, before any door geometry, so mouths
    // are laid out on the post-fusion wall rather than on a barrier the splice
    // later replaces. `areas` is final after `ruins::build`, so this sees the
    // same input it would later. The outline-safety filter needs the traced
    // outline (which needs jambs, which needs mouths), so it stays downstream —
    // door placement therefore sees the *unfiltered* set, which is harmless:
    // avoiding a neck that is later dropped only nudges a door slightly.
    // Class B (level) and class C (upright) corridors from the one construction,
    // then the class-A angle necks.
    // The two hex diagonals: corridor axis ∓30°, normal 60°/120°. Which one a pair
    // uses is chosen per pair inside `axis_necks` (the FIX diagrams' mirror rule).
    const AP: f64 = grid::HEX_APOTHEM;
    const DIAGONALS: [(Point, Point); 2] = [((AP, -0.5), (0.5, AP)), ((AP, 0.5), (-0.5, AP))];
    // Classify every fused pair ONCE (pre-claim cells); all connector builders
    // and the class-D barrier list share this table.
    let pairs = fuse_pairs(&areas);
    let mut all_necks =
        axis_necks(&areas, &pairs, FuseClass::Horiz, &[((1.0, 0.0), (0.0, 1.0))], oparams.hex_size);
    all_necks.extend(axis_necks(
        &areas,
        &pairs,
        FuseClass::Vert,
        &[((0.0, 1.0), (1.0, 0.0))],
    ...
    all_necks.extend(axis_necks(&areas, &pairs, FuseClass::Both, &DIAGONALS, oparams.hex_size));
    // Class A for rect↔rect and circle↔circle. Verified against the FIX diagram:
    // these corner pairs are geometrically near-stacked (they overlap on one axis
    // with a small gap on the other), so the axis clamp already lands the walls on
    // the right lines — the diagram's "L" is just the junction with each room's own
    // edge, which the outline traces anyway. No separate L geometry needed.
    all_necks.extend(axis_necks(
        &areas,
        &pairs,
        FuseClass::Angle,
    ...
    all_necks.extend(circle_rect_necks(&areas, &pairs, oparams.hex_size));
    ...
    // projects onto its own room wall. Collect the touching cells as a "neck":
    // the outline locks these on their raw hex corners instead — a full-width,
    // hex-aligned connection between the two rooms (see `smooth_loops`). Wide
    // touches (≥3 faces) already read as one compound and are left alone.
    let mut neck_cells = fused_necks(&areas);
    let barrier_pairs = class_both_pairs(&areas, &pairs);
    // Give each corridor floor to stand on, so a wide clamp stops degrading down
    // the ladder for want of it. Claimed cells join the neck set: like a narrow
    // seam's cells they belong to the join rather than to either room, so the
    // outline must lock them on their own hex corners instead of projecting them
    // onto a room wall (which would undo the fill).
    let claimed =
        claim_corridor_floor(&mut areas, &grid, &topology, &mut all_necks, oparams.hex_size);
    neck_cells.extend(claimed.values().flatten().copied());
    ...
    // line.
    // Only a NARROW connector nudges a door aside (bounded to one opening — see
    // `slide_clear`). A wide class-B corridor spans a whole flank, and moving a
    // door for one can fold the traced outline at the jamb *however small* the
    // move — measured on seed 9, where any displacement folds it. Those yield to
    ...
    let mouths = doorway::mouths(&topology, &areas, oparams.hex_size, &|p| {
        all_necks.iter().filter(|n| !n.is_corridor()).any(|n| n.blocks(p))
    });
    ...
    // A wide connector yields to any door already cut where its wall would go —
    // the fusion falls back to the band merge for that pair rather than erasing a
    // doorway or shoving it far enough to fold the outline at the jamb.
    let all_necks: Vec<Neck> = all_necks
        .into_iter()
        .filter(|n| {
            !n.is_corridor()
                || !mouths.iter().any(|mo| {
                    ...
                    [-1.0, 0.0, 1.0].iter().any(|f| {
                        n.on_new_wall(
                            ...
                            oparams.hex_size / 2.0,
                        )
                    })
                })
        })
        .collect();
    let necks = splice_outline_necks(&mut outline, all_necks);
    // Door cells (and the pillar a merged pair swallows) locate the doorway gaps a
    // connector's wall has to break for — see `Neck::jamb_edge`.
    let door_cells: Vec<Point> = topology
        .doors
        .iter()
        .map(|d| d.cell.center(oparams.hex_size))
        .chain(topology.merged_doors.iter().map(|&(_, _, p)| p.center(oparams.hex_size)))
        .collect();
    splice_necks(&mut dungeon_walls, &necks, &door_cells);
    crop_internal_barriers(&mut dungeon_walls, &barrier_pairs);
    // The corridor is settled now, so give back any claimed floor its accepted
    // walls do not enclose — before water, stones and the floor pattern read the
    // cell set below.
    release_unused_claims(&mut areas, &necks, &claimed, oparams.hex_size);
